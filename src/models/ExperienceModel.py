...
class ExperienceModel(BaseDataModel):
    ...
    async def get_user_experiences(self, user_id: int, page_no:int=1, page_size: int=50 ):
        async with self.db_client() as session:
            stmt = select(Experience).where(Experience.experience_user_id == user_id).offset((page_no - 1) * page_size).limit(page_size)
